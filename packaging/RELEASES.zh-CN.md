...
源码版本：`690bd530`（`build: reject dirty worktree in build.sh; record source commit in releases`），构建自该提交的干净工作区。
...
构建自源码版本 `690bd530`，包内自带 `BUILD-INFO.txt`。产物位于
...
- **Extras 包**（同 commit）：macOS 干净 HOME ＋ Linux `--network none` 容器——安装退出码 0、重跑幂等；`run_cli_app` 成功调起三个 CLI（`dct-north-cli`、`cli-anything-asset-historical-data`、`chart`）；6 个 skill 加载无重复、无 `dct-north-cli` 残留；MCP server 能启动并列出 3 个 tools（网关日志 "connected, 3 capabilities registered"）；`installed.json` 无开发机路径，MCP `command` 指向自带 Python。两次运行也重跑了主包冒烟。
- 验证 extras 时发现并修复：无注册表缓存且无网的机器上，`run_cli_app` 在到达本地回退前崩溃（上游对必需目录源重新抛出）。已在 `get_app()` 修复 —— `# FORK-HOOK: fork-cli-apps-offline-catalog`。

### 已知限制

- 不预装第三方 CLI App；内部 CLI/skill/MCP 走独立的 `nanobot-extras-*` 包（见 offline-extras spec）。
